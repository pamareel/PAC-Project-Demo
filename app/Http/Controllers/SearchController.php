...
class SearchController extends Controller
{
    public function index(){
        ...
        if(empty($resultSearch)){
            ...
            $avg = NULL;
            $cor_r1= NULL; $cor_r2= NULL; $cor_r3= NULL; $cor_r4= NULL; $cor_r5= NULL; $cor_r6= NULL; $cor_r7= NULL;
            $cor_r8= NULL; $cor_r9= NULL; $cor_r10= NULL; $cor_r11= NULL; $cor_r12= NULL; $cor_r13= NULL;

            $cor_r1_value= NULL; $cor_r2_value= NULL; $cor_r3_value = NULL; $cor_r4_value= NULL; $cor_r5_value= NULL;
            $cor_r6_value= NULL; $cor_r7_value= NULL; $cor_r8_value = NULL; $cor_r9_value= NULL; $cor_r10_value= NULL;
            $cor_r11_value= NULL; $cor_r12_value= NULL; $cor_r13_value= NULL;
    ...
    function chart_Low_Med_High_All($countHosAll,$year,$GT,$Dname,$method){
        ...
        $query_AVG = "select cast(AVG(PAC_value) as decimal(18,3)) as avg from [PAC_hos_".$GT."] where BUDGET_YEAR = '".$year."' and ".$GT."_NAME LIKE '".$Dname."'"; 
    ...
    function find_Map_Data($r,$y,$g,$na,$m){
        if($r == 'All'){
            if($m == 'All'){
                ////// Thai map //////////////////////////////////////////////////////////////////////
                $query_rd = "select Region, cast(sum(CAST(Total_Amount as float) * CAST(wavg_Unit_Price as float))/sum(CAST(Total_Amount as float)) as decimal(10,2)) as wavg_unit_price, sum(Total_Amount) as Total_Amount, FORMAT(sum(Total_Amount), N'N2') as T_Total_Amount from [PAC_hos_".$g."] where BUDGET_YEAR = '".$y."' and ".$g."_NAME LIKE '".$na."' group by Region order by Region";
            }else{
                ////// Thai map //////////////////////////////////////////////////////////////////////
        ...
        if($m == 'All'){
            if($r == 1){
                foreach($Region_1_name as $Pcode => $Province){
                    $query_rd = "select DEPT_ID, DEPT_NAME, ServicePlanType, IP, FORMAT(IP, N'N2') as I_IP, OP, FORMAT(OP, N'N2') as O_OP, Total_Amount, FORMAT(Total_Amount, N'N2') as T_Total_Amount, wavg_unit_price, cast(wavg_unit_price as decimal(10,2)) as w_wavg_unit_price, Total_Spend, FORMAT(Total_Spend, N'N2') as T_Total_Spend, cast(PAC_value as decimal(10,2)) as PAC_value from [PAC_hos_".$g."] where BUDGET_YEAR = '".$y."' and ".$g."_NAME LIKE '".$na."' and PROVINCE_EN = '".$Province."' order by PAC_value ";
    ...
    function Donut_Type_Hos($year,$GT,$Dname,$method){
        ...
        foreach($size_hospital_name as $s){
            if($method == 'All'){
                $countquery_r = "select Count(DEPT_ID) as num_hos, cast(SUM(Total_Amount * [wavg_unit_price]) / SUM(Total_Amount) as decimal(18,3)) AS Wavg_unit_price, sum(Total_Amount) as Total_Total_Amount, FORMAT(sum(Total_Amount), N'N2') as Total_Amount from [PAC_hos_".$GT."] where BUDGET_YEAR = '".$year."' and ".$GT."_NAME LIKE '".$Dname."' and ServicePlanType = '".$s."';";
            }else{
        ...
        if($method == 'All'){
            $q = "select FORMAT(sum(Total_Amount), N'N2') as Total_Amount from [PAC_hos_".$GT."] where BUDGET_YEAR = '".$year."' and ".$GT."_NAME LIKE '".$Dname."';";
        }else{
            $q = "select FORMAT(sum(Total_Amount), N'N2') as Total_Amount from [PAC_hos_".$GT."] where BUDGET_YEAR = '".$year."' and ".$GT."_NAME LIKE '".$Dname."' and Method ='".$method."';";
        }
